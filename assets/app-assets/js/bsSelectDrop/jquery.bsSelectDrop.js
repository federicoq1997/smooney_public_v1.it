(function ($) {
		...
		function init($select) {
			...
			let $dropdown = $('<div>', {
				...
				html: `
                <a class="${settings.btnClass} dropdown-toggle d-flex flex-nowrap align-items-center" ${closeOutside} href="javascript:;" role="button" data-bs-toggle="dropdown"
			...
			let actionMenu = '';
			if (settings.search) {
				searchInput = `<input type="search" autocomplete="off" class="form-control form-control-sm me-auto" placeholder="Search..">`;
			}
				...
				if (settings.showActionMenu) {
					actionMenu = `
						<div class="btn-group btn-group-sm mt-2 p-0">
							<a href="javascript:;" class="btn btn-link p-0 js-select-select-all">${settings.selectAllText}</a>
							<a href="javascript:;" class="btn btn-link p-0 js-select-select-none">${settings.deselectAllText}</a>
						</div>
					`;
				}
			}
			...
			$select.find('optgroup, option').each(function (index, option) {
				...
				$('<div>', {
					...
					class: classList,
					html: `
                <a class="dropdown-item ${selected} ${disabledClass} d-flex align-items-end" data-index="${i}" href="javascript:;">
